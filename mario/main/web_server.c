...
static void http_server(struct netconn *conn) {
	...
	// mario0.png
	extern const uint8_t mario0_png_start[] asm("_binary_mario0_png_start");
	extern const uint8_t mario0_png_end[] asm("_binary_mario0_png_end");
	const uint32_t mario0_png_len = mario0_png_end - mario0_png_start;

	// mario1.png
	extern const uint8_t mario1_png_start[] asm("_binary_mario1_png_start");
	extern const uint8_t mario1_png_end[] asm("_binary_mario1_png_end");
	const uint32_t mario1_png_len = mario1_png_end - mario1_png_start;
	...
	if(err==ERR_OK) {
		...
		if(buf) {
			...
			else if(strstr(buf,"GET /mario0.png ")) {
				ESP_LOGI(TAG,"Sending mario0.png");
				netconn_write(conn,PNG_HEADER,sizeof(PNG_HEADER)-1,NETCONN_NOCOPY);
				netconn_write(conn,mario0_png_start,mario0_png_len,NETCONN_NOCOPY);
				netconn_close(conn);
				netconn_delete(conn);
				netbuf_delete(inbuf);
			}

			else if(strstr(buf,"GET /mario1.png ")) {
				ESP_LOGI(TAG,"Sending mario1.png");
				netconn_write(conn,PNG_HEADER,sizeof(PNG_HEADER)-1,NETCONN_NOCOPY);
				netconn_write(conn,mario1_png_start,mario1_png_len,NETCONN_NOCOPY);
				netconn_close(conn);
				netconn_delete(conn);
				netbuf_delete(inbuf);
			}
